...
DATA_FORMAT = 'NHWC'
config = namedtuple('config', ['eval_dir',
                               'dataset_dir',
                               'dataset_split_name',
                               'checkpoint_path',
                               'model_name',
...
FLAGS = config(
    ...
    batch_size=1,
    num_classes=21,
    num_preprocessing_threads=4,  # 创建batch使用的线程数量
    matching_threshold=0.5,  # groundtruth匹配阈值
    nms_threshold=0.45,  # Non-Maximum阈值
    wait_for_checkpoints=False,  # 验证时是否等待新的checkpoint
    master='',  # The address of the TensorFlow master to use
    max_num_batches=500,  # Max number of batches to evaluate by default use all.
